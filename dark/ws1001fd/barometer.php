...
if ($barometer === $trendbarometer) {
    echo "<span style='color:#ccc;'>trend<span>
	<i class='wi wi-wind towards-90-deg'></i></span>
	<steady>steady</steady>";
}
 if ($barometer > $trendbarometer) {
    echo "<span style='color:#ccc;'>rising</span><span style='color:#f26c4f;'>
	<i class='wi wi-wind towards-45-deg'></i>
	steadily</span>";
}

if($barometer < $trendbarometer) {
    echo "<span style='color:#ccc;'>falling</span><span style='color:#607d8b;'>
...
$("#circularGaugeContainer").dxCircularGauge({
  ...
  scale: {
    startValue:28,
	endValue:31,
    majorTick: { tickInterval: 0.5, visible: true, color: 'rgba(255,255,255,0)'},
	minorTick: {color: 'rgba(255,255,255,0)',visible: true,minorTickInterval: 0.25},
	label: {
	format: '',
